@YarnContainerLogFetcherScenario
Feature: YarnContainerLogFetcher unit tests
  Scenario: parsingAmHostHttpAddressHost unit tests
    Given prepare a Yarn cluster with Node Manager base URL http://127.0.0.1:$port/yarnui/hn/ and UI base URL http://127.0.0.1:$port/yarnui/
  ...
  Scenario: parse logs from history server HTML page
    Given prepare a Yarn cluster with Node Manager base URL http://127.0.0.1:$port/yarnui/ws/v1/cluster/apps/ and UI base URL http://127.0.0.1:$port/yarnui/
    And create a yarn application driver with id mockId
    And parse 'stdout' from Yarn container log fetched HTML page 'YarnContainerLog01.html'
    Then check the type log 'directory.info' should start with '.template'
    Then check the type log 'prelaunch.err' should not exist
    Then check the type log 'prelaunch.out' should not exist
  ...
  Scenario: parse logs from history server HTML page 03
    Given prepare a Yarn cluster with Node Manager base URL http://127.0.0.1:$port/yarnui/ws/v1/cluster/apps/ and UI base URL http://127.0.0.1:$port/yarnui/
    And create a yarn application driver with id mockId
    And parse 'stdout' from Yarn container log fetched HTML page 'YarnContainerLog03.html'
    Then check the type log 'stdout' should not exist

  Scenario: parse logs from running container HTML page
    Given prepare a Yarn cluster with Node Manager base URL http://127.0.0.1:$port/yarnui/ws/v1/cluster/apps/ and UI base URL http://127.0.0.1:$port/yarnui/
    And create a yarn application driver with id mockId
    And parse 'stdout' from Yarn container log fetched HTML page 'YarnContainerLog02.html'
    Then check the type log 'stdout' should start with '(10.10.10.10'

  Scenario: parse logs from an empty HTML page
    Given prepare a Yarn cluster with Node Manager base URL http://127.0.0.1:$port/yarnui/ws/v1/cluster/apps/ and UI base URL http://127.0.0.1:$port/yarnui/
    And create a yarn application driver with id mockId
    Then parse 'stdout' from Yarn container log fetched HTML ''
